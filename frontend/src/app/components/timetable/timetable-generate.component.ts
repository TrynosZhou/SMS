...
@Component({
  standalone: false,  selector: 'app-timetable-generate',
  templateUrl: './timetable-generate.component.html',
  styleUrls: ['./timetable-generate.component.css']
})
export class TimetableGenerateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.isViewMode = this.route.snapshot.data['timetableMode'] === 'view';
    const pagePath = this.isViewMode ? '/timetable/view' : '/timetable/generate';
    ...
    forkJoin({
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.error = 'Failed to load teachers, classes, or subjects';
          this.loading = false;
          this.cdr.markForCheck();
        }
      });
