...
export class RecordBookComponent implements OnInit {
  ...
  loadTeacherInfo() {
    ...
    this.teacherService.getCurrentTeacher().subscribe({
      next: (teacher: any) => {
        ...
        console.log('Subjects count:', teacher.subjects?.length || 0);
        if (teacher.subjects && teacher.subjects.length > 0) {
          console.log('Subject names:', teacher.subjects.map((s: any) => s.name).join(', '));
        } else {
          console.log('WARNING: No subjects assigned to this teacher. Using fallback mode.');
        }
  ...
  loadTeacherSubjectsForClass(classData: any) {
    ...
    const teacherHasSubjects = this.teacher && this.teacher.subjects && this.teacher.subjects.length > 0;
    
    if (teacherHasSubjects) {
      // Filter to show only subjects that both teacher and class have
    ...
    // FALLBACK: If teacher has no subjects assigned OR no matching subjects found,
    // show all subjects from the class so teacher can still enter marks
    // This handles the case where teacher is assigned to class but subjects not configured
    console.log('Using fallback: showing all class subjects for teacher');
    this.availableSubjects = classData.subjects;
    this.teacherSubjects = this.availableSubjects.map((s: any) => s.name).join(', ');
    
    // Show info message about fallback
    if (!teacherHasSubjects) {
      console.log('Note: Teacher has no subjects assigned. Showing all class subjects.');
    } else {
      console.log('Note: No matching subjects between teacher and class. Showing all class subjects.');
    }
