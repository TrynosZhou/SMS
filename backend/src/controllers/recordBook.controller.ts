...
export const getRecordBookByClass = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!skipAssignmentCheck) {
      ...
      // Check if subject is taught in this class (allows teacher to work with class subjects)
      ...
      // Allow access if: subject is in class OR teacher has subject assigned
      const teachesSubject = teacher.subjects?.some(s => s.id === subjectId);
      
      if (!subjectInClass && !teachesSubject) {
