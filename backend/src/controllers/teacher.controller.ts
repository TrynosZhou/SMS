...
export const getCurrentTeacher = async (req: AuthRequest, res: Response) => {
  try {
    ...
    try {
      ...
      if (username) {
        // Find ALL teachers with matching EmployeeID (case-insensitive) - include subjects relation
        ...
        if (allMatchingByEmployeeId.length > 0) {
          ...
          if (preferred) {
            ...
            console.log('[getCurrentTeacher] Teacher subjects:', preferred.subjects?.map(s => s.name).join(', ') || 'none');
          } else {
