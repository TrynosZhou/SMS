...
export const getReportCard = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isTeacher && user?.teacher?.id) {
      ...
      // Verify teacher teaches this subject OR subject is in the class
      if (subjectId) {
        const teachesSubject = teacher.subjects?.some(s => s.id === subjectId);
        // Also check if subject is assigned to the class
        const classEntity = await classRepository.findOne({
          where: { id: classId as string },
          relations: ['subjects']
        });
        const subjectInClass = classEntity?.subjects?.some(s => s.id === subjectId);
    ...
    if (exams.length === 0) {
      ...
      if (requirePublished && hasRequestedType && !hasPublishedRequestedType) {
...
export const generateMarkSheet = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isTeacher && user?.teacher?.id) {
      ...
      // If a specific subject is requested, verify teacher teaches this subject OR subject is in class
      if (subjectId) {
        const teachesSubject = teacher.subjects?.some(s => s.id === subjectId);
        const subjectInClass = classEntity?.subjects?.some(s => s.id === subjectId);
